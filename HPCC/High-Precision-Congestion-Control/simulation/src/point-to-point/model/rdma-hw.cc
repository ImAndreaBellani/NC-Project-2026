...
void RdmaHw::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish){
	...
	}else if (m_cc_mode == 3 || m_cc_mode == 30 || m_cc_mode == 300){
...
int RdmaHw::ReceiveAck(Ptr<Packet> p, CustomHeader &ch){
	...
	if (m_cc_mode == 3){
		HandleAckHp(qp, p, ch, true, true);
	}else if (m_cc_mode == 30){
		HandleAckHp(qp, p, ch, true, false);
	}
	else if (m_cc_mode == 300){
		HandleAckHp(qp, p, ch, false, true);
	}
	else if (m_cc_mode == 7){
 ...
 * High Precision CC
 ***********************/
void RdmaHw::HandleAckHp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool per_RTT, bool per_ACK){
	...
	if (per_RTT){
		if (per_ACK){
			...
			}else{ // do fast react //redirect to set fast_react = true
