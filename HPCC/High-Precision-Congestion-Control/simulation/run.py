...
if __name__ == "__main__":
	...
	if (args.cc.startswith("dcqcn")):
	...
	elif args.cc == "hp" or args.cc == "hr" or args.cc == "ha":
		ai = 10 * bw / 25;
		if args.hpai > 0:
			ai = args.hpai
		hai = ai # useless
		...
		mode_hp = (args.cc == "hp")*3 + (args.cc == "hr")*30 + (args.cc == "ha")*300

		config = config_template.format(bw=bw, trace=trace, topo=topo, cc=cc, mode=mode_hp, t_alpha=1, t_dec=4, t_inc=300, g=0.00390625, ai=ai, hai=hai, dctcp_ai=1000, has_win=1, vwin=1, us=1, u_tgt=u_tgt, mi=mi, int_multi=int_multi, pint_log_base=pint_log_base, pint_prob=pint_prob, ack_prio=0, link_down=args.down, failure=failure, kmax_map=kmax_map, kmin_map=kmin_map, pmax_map=pmax_map, buffer_size=bfsz, enable_tr=enable_tr)
	elif args.cc == "dctcp":
