...
if __name__ == "__main__":
	...
	n_flow = 0
	print("poisons avg_inter_arrival: %f avg_inter_incast: %f sum: %f"%(int(poisson(avg_inter_arrival)), int(poisson(avg_inter_incast)),int(poisson(avg_inter_incast+avg_inter_arrival))))
	print("avg_inter_arrival: %f avg_inter_incast: %f sum: %f"%(avg_inter_arrival, avg_inter_incast, avg_inter_incast+avg_inter_arrival))
	ofile.write("%d \n\n"%n_flow_estimate)
	host_list = [(base_t + int(poisson(avg_inter_arrival)), i) for i in range(nhost)]
	heapq.heapify(host_list)
	while len(host_list) > 0:
		t,src = host_list[0]
		inter_t_avg = int(poisson(avg_inter_arrival))
		inter_t_incast = 0#int(poisson(avg_inter_incast))
		if inter_t_avg > inter_t_incast :
			inter_t = inter_t_avg
