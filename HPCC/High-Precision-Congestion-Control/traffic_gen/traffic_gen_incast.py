...
if __name__ == "__main__":
    ...
    if not options.nhost:
        print
        "please use -n to enter number of hosts"
        sys.exit(0)
    ...
    avg_inter_incast = 1 / (bandwidth * nhost * 0.02 / 8 / (500000 * 60)) * 1000000000
    ...
    n_flow_estimate = int(time / avg_inter_arrival * nhost) + int(time / avg_inter_incast)
    print("n_flow_estimate: %d" % n_flow_estimate)
    n_flow = 0
    print("poisons avg_inter_arrival: %f avg_inter_incast: %f sum: %f" % (
    int(poisson(avg_inter_arrival)), int(poisson(avg_inter_incast)),
    int(poisson(avg_inter_incast + avg_inter_arrival))))
    print("avg_inter_arrival: %f avg_inter_incast: %f sum: %f" % (
    avg_inter_arrival, avg_inter_incast, avg_inter_incast + avg_inter_arrival))
    ofile.write("%d \n\n" % n_flow_estimate)
    host_list = [(base_t + int(poisson(avg_inter_arrival)), i) for i in range(nhost)]
    print("host_list: %d" % (len(host_list)))
    all_flows = []
    ...
    incast_list = [(base_t + int(poisson(avg_inter_incast)), random.randint(0, nhost - 1)) for i in
                   range(int(time / avg_inter_incast) + 1)] #questo forza la a stargli dentro l'expcted incasts (nonfunziona bene e non era la versione originale)
    heapq.heapify(incast_list)
    while len(incast_list) > 0:
        t, dst = incast_list[0]
        inter_t = int(
            poisson(avg_inter_incast))  # moltiplicare per 60 qua aiuta ma non capisco perch negli altri punti meglio
