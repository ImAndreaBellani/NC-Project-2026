...
* it often takes months to tune the parameters for RDMA before actual deployment, in order to find a good balance between high performances and stability. Moreover, because different applications have different traffic patterns, and different environments have different topologies, link speeds, and switch buffer sizes, operators have to tune parameters for the deployment of each new application and new environment.
